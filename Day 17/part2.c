...
#	define NODE_MAX 40
	...
	while (activeTileCount > 0) {
		...
		if (activeNode->x < width && activeNode->y < height) {
			// Check if found.
			if (activeNode->distance <= 0 && activeNode->directionCount >= 4) {
			...
			for (size_t i = 0; i < sizeof(directions) / sizeof(*directions); i++) {
				...
				// Check move distance.
				if (activeNode->direction == checkDirection && activeNode->directionCount >= 10) continue;
				if (activeNode->direction != checkDirection && activeNode->directionCount < 4 && activeNode->directionCount != 0) continue;
