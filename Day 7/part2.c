...
int main() {
	...
	for (size_t i = 0; i < handCount; i++) {
		...
		if (false)
			printf(
				"Hand type: %zu \t Cards: %c %c %c %c %c \t Bet: %zu \t Rank: %zu \t Winnings: %6zu \t Total: %zu\n",
				hand->m_type,
				hand->m_cards[0].m_char, hand->m_cards[1].m_char, hand->m_cards[2].m_char, hand->m_cards[3].m_char, hand->m_cards[4].m_char,
				hand->m_betAmount,
				rank, winnings,
				totalWinnings
			);
...
const bool Card_isSame(const enum CardType a, const enum CardType b) {
	return a == b || a == CardType_Joker || b == CardType_Joker;
}
const enum CardType Card_getCardValue(const char cardChar) {
	switch (cardChar) {
...
const enum HandType Hand_getHandType(const struct Hand* const this) {
	...
	for (size_t i = 0; i < 5; i++)
		if (sortedCards[i] == CardType_Joker)
			jokerCount++;

	switch (jokerCount) {
		case 5:
		case 4: return HandType_FiveOfAKind;
		case 3: return (Card_isSame(sortedCards[0], sortedCards[1])) ? HandType_FiveOfAKind : HandType_FourOfAKind;

		case 2: {
			if (Card_isSame(sortedCards[0], sortedCards[1]) &&
				Card_isSame(sortedCards[1], sortedCards[2]))
		...
		case 1: {
			if (Card_isSame(sortedCards[0], sortedCards[1]) &&
				Card_isSame(sortedCards[1], sortedCards[2]) &&
				Card_isSame(sortedCards[1], sortedCards[3]))
				return HandType_FiveOfAKind;
			if ((Card_isSame(sortedCards[0], sortedCards[1]) && Card_isSame(sortedCards[1], sortedCards[2])) ||
				(Card_isSame(sortedCards[1], sortedCards[2]) && Card_isSame(sortedCards[2], sortedCards[3])))
				return HandType_FourOfAKind;
			if (Card_isSame(sortedCards[0], sortedCards[1]) && Card_isSame(sortedCards[2], sortedCards[3]))
				return HandType_FullHouse;
			if (Card_isSame(sortedCards[0], sortedCards[1]) ||
				Card_isSame(sortedCards[1], sortedCards[2]) ||
				Card_isSame(sortedCards[2], sortedCards[3]))
				return HandType_ThreeOfAKind;
			...
			for (size_t i = 1; i < 4; i++) {
				// Pair found.
				if (Card_isSame(sortedCards[i], currentCard))
