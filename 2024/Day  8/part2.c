...
int main() {
	...
	for (int c = 0; c < ANTENNA_CHARS; c++) {
		...
		for (int i = 0; i < antennaCounts[c]; i++) {
			// Mark antenna as antinode.
			map[antennaPositions[c][i].x][antennaPositions[c][i].y] = '#';
			for (int j = i + 1; j < antennaCounts[c]; j++) {
				...
				Point checks[] = { Point_subtract(&p1, &change), Point_add(&p2, &change) };
				// Keep checking until both antinodes points are out of the map.
				int outOfRangeCount = 0;
				while (outOfRangeCount < ARRAY_SIZE(checks)) {
					outOfRangeCount = 0;
					// Check points.
					...
					// Move checks.
					checks[0] = Point_subtract(&checks[0], &change);
					checks[1] = Point_add(&checks[1], &change);
				}
			}
		}
	}
