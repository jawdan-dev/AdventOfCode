...
int main() {
	...
	// Get best total traversed.
	size_t bestTotalTraversed = 0;
	for (size_t i = 0; i < width + height; i++) {
		// Accross top/bottom, accross sides.
		for (size_t j = 0; j < 2; j++) {
			// Get x, y, and direction based on i + j.
			const size_t x = i < width ? i : (j < 1 ? 0 : width - 1);
			const size_t y = i >= width ? i - width : (j < 1 ? 0 : height - 1);
			const enum CurrentDirection beamDirection =
				i < width ?
				(j < 1 ? CurrentDirection_South : CurrentDirection_North) :
				(j < 1 ? CurrentDirection_East : CurrentDirection_West);

			// Get total traversed.
			const size_t totalTraversal =
				getTotalTraversed(world, width, height, x, y, beamDirection);

			// Update max.
			if (bestTotalTraversed < totalTraversal)
				bestTotalTraversed = totalTraversal;
		}
	}
