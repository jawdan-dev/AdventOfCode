...
int main() {
	...
	size_t safeToDisintegrateCount = 0;

	for (size_t n = 0; n < brickCount; n++) {
		bool safeToDisintegrate = true;

		for (size_t i = 0; safeToDisintegrate && i < brickCount; i++) {
			if (i == n) continue;
			...
			if (canMove) safeToDisintegrate = false;
		}

		// Count.
		if (safeToDisintegrate) safeToDisintegrateCount++;
	}
